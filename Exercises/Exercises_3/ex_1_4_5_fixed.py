...
df_cleaned = df
seaborn.pairplot(df_cleaned, hue='species') #hue = different colors for different species
#plt.show()
...
pipe_randomForest = make_pipeline(
    StandardScaler(),
    RandomForestClassifier(n_estimators=25, random_state=42,max_depth=3) 
)

features = df_cleaned[['bill_depth_mm', 'flipper_length_mm']].copy()
features_rotated = df_cleaned[['flipper_length_mm', 'bill_depth_mm']].copy()
featuer_all = df_cleaned[['bill_length_mm', 'bill_depth_mm', 'flipper_length_mm', 'body_mass_g']].values.copy()
...
score = pipe_randomForest.score(X_test, y_test)         #Genauigkeit Random Forest:  same by rotated features
